...
plate_processor = None  # 延迟初始化（因为加载AI模型需要时间）


def init_processor():
    """初始化车牌识别处理器"""
    global plate_processor
    if plate_processor is None:
        try:
            plate_processor = PlateRecognitionProcessor(device_manager)
...
def register_device():
    ...
    try:
        ...
        if success:
            # 启动车牌识别处理
            if plate_processor:
                plate_processor.start_processing(device_id, stream_url)
                message = "设备注册成功，AI车牌识别已启动"
            else:
...
def unregister_device():
    ...
    try:
        ...
        # 停止车牌识别处理
        if plate_processor:
            plate_processor.stop_processing(device_id)
...
def get_plate_results():
    ...
    try:
        ...
        results = plate_processor.get_latest_results(max_count=limit)
...
def health_check():
    ...
    return jsonify({
        "status": "ok",
        "message": "云端服务运行正常",
        "ai_enabled": plate_processor is not None
    }), 200
