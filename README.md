...
- 前端可在视频中叠加异常物品框
- **技术方案**: DINOv2固定机位正常模板 + YOLO车辆掩膜 + 道路区域约束 + 时序异常判定（MOG2可回退）
...
- 车牌识别: LPRNet
- 目标跟踪: ByteTrack
- 异常检测: DINOv2正常模板特征对比 + 车辆掩膜 + 道路区域约束 + 时序判定
...
- 背景学习允许少量车辆经过，车辆区域会被遮罩；遮挡道路过多时才跳过当前帧
- DINOv2逐位置比较正常道路参考特征与当前帧，识别任意新增物体
- YOLO车辆掩膜消除正常车辆遮挡干扰
- drivable_area道路分割或道路ROI限制检测范围
- 时序追踪确认持续占用道路的未知异常物体
- 连续检测到全局画面变化时停止告警并自动返回背景标定，避免镜头移动造成满屏误报
